...
`HostClient.OpenStream`. Physical connection hijacking remains an HTTP/1
operation: `RequestCtx.TryHijack` returns `ErrHijackNotSupported` for a
multiplexed request, and legacy `Hijack` produces a 501 response instead of a
silent no-op.

HTTP/3 is not implemented. [`http2/DESIGN.md`](http2/DESIGN.md) covers the
concurrency model, the resource limits, and the defaults worth arguing
about.
...
- _Does fasthttp support HTTP/2.0 and WebSockets?_
  ...
  on HTTP/1 connections; HTTP/2 bidirectional protocols use
  `RequestCtx.AcceptStream` and `HostClient.OpenStream`.
